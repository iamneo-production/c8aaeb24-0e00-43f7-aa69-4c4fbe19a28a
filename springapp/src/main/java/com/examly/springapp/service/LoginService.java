...
public class LoginService {
    ...
    public ResponseEntity<?> forgot(String email) {
        ...
        if (userModel == null) {
            regularAuditService
                    .audit(new RegularAuditModel("Request for forgot password", email, "No user for found", false));
            errors.add("User not found");
            return ResponseEntity.ok().body(
                    new ApiResponse(false, "No user found for given email", FORBIDDEN.value(), FORBIDDEN, errors));
        } else {
            ...
            try {
                regularAuditService.audit(new RegularAuditModel("Request for forgot password", email,
                        "Mail with code has been sent", true));
                emailSenderService.sendMail(userModel.getEmail(), "Ebook Store - Reset your password", body);
            } catch (MessagingException e) {
                e.printStackTrace();
                errors.add(e.getLocalizedMessage());
                regularAuditService.audit(new RegularAuditModel("Request to load user", email,
                        "There was an issue in sending mail", false));
                return ResponseEntity.ok().body(new ApiResponse(false, "There was an issue in sending mail",
                        FORBIDDEN.value(), FORBIDDEN, errors));
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
                errors.add("The encoding of mail is not supported");
                regularAuditService.audit(new RegularAuditModel("Request to load user", email,
                        "The encoding of mail is not supported", false));
                return ResponseEntity.ok().body(new ApiResponse(false, "There was an issue in sending mail",
                        FORBIDDEN.value(), FORBIDDEN, errors));
            }
            ...
            regularAuditService.audit(new RegularAuditModel("Request to load user", email,
                    "Mail was sent and forgot password is saved", true));
            return ResponseEntity.ok().body(new ApiResponse(true,
                    "The mail with code has been sent, please check your inbox", OK.value(), OK, errors));
        }
    }
    ...
    public ResponseEntity<?> verifyCode(String email, String code) {
        ...
        if (userModel == null) {
            errors.add("User not found");
            regularAuditService
                    .audit(new RegularAuditModel("Request to verify code for user", email, "No user found", false));
            return ResponseEntity.ok().body(
                    new ApiResponse(false, "No user found for given email", FORBIDDEN.value(), FORBIDDEN, errors));
        } else {

            if (userModel.getForgotCode().equals(code)) {
                regularAuditService.audit(new RegularAuditModel("Request to verify code for user", email,
                        "The code was validated", true));
                return ResponseEntity.ok().body(new ApiResponse(true,
                        "The given code was validated, please enter your new password", OK.value(), OK, errors));
            } else {
                errors.add("Invalid code");
                regularAuditService.audit(new RegularAuditModel("Request to load user", email,
                        "Invalid code provided for this user", false));
                return ResponseEntity.ok().body(new ApiResponse(false, "Invalid code provided for this user",
                        FORBIDDEN.value(), FORBIDDEN, errors));
            }
        }
    }
    ...
    public ResponseEntity<?> savePassword(String email, String password, String conformPassword, String code)
        ...
        if (!password.equals(conformPassword)) {
            ...
            regularAuditService.audit(
                    new RegularAuditModel("Request to save password", email, "Both passwords are not equal", false));
            return ResponseEntity.ok()
                    .body(new ApiResponse(false, "Both passwords doesn't match", FORBIDDEN.value(), FORBIDDEN, errors));
        } else {
            UserModel userModel = userRepository.findByEmail(email);
            if (userModel == null) {
                errors.add("User not found");
                regularAuditService
                        .audit(new RegularAuditModel("Request to save password", email, "No user found", false));
                return ResponseEntity.ok().body(
                        new ApiResponse(false, "No user found for given email", FORBIDDEN.value(), FORBIDDEN, errors));
            } else {
                if (!code.equals(userModel.getForgotCode())) {
                    errors.add("Wrong code");
                    regularAuditService.audit(new RegularAuditModel("Request to save password", email,
                            "Invalid code provided for this user", false));
                    return ResponseEntity.ok().body(new ApiResponse(false, "Invalid code provided for this user",
                            FORBIDDEN.value(), FORBIDDEN, errors));
                } else {
                    ...
                    return ResponseEntity.ok()
                            .body(new ApiResponse(true, "Password changed successfully", OK.value(), OK, errors));
                }
            }
        }
    }
