...
public class CartService {
    ...
    public ResponseEntity<Object> addCart(@Valid CartModel cart) {
        ...
        if (cartModel != null) {
            ...
            regularAuditService.audit(new RegularAuditModel("Request to add new cart",  email, "Cart already exists", false));
        ...
        try {
            ...
            if (errors.size() > 0)
            {
                regularAuditService.audit(new RegularAuditModel("Request to add new cart",  email, "Validation failed", false));
        ...
        try {
            regularAuditService.audit(new RegularAuditModel("Request to add new cart",  email, "Attempting to save", true));
            cartRepository.save(cart);
        } catch (DataIntegrityViolationException e) {
            if (e.getMessage().contains("constraint")) {
                regularAuditService.audit(new RegularAuditModel("Request to add new cart",  email, "Constraint error was caused while adding", false));
                errors.add(e.getMessage());
                return new ResponseEntity<Object>(new ApiResponse(false, "Constraint Error", NOT_ACCEPTABLE.value(), NOT_ACCEPTABLE, errors), HttpStatus.OK);
            }
        }
    ...
    public ResponseEntity<?> addProduct(String id, ProductTempModel productTempModel) {
        ...
        try {
            if (userModel == null) {
                ...
                regularAuditService.audit(new RegularAuditModel("Request to add product to cart",  email, "User not found", false));
            ...
            if (!email.equals(userModel.getEmail())) {
                ...
                regularAuditService.audit(new RegularAuditModel("Request to add product to cart",  email, "Invalid request for this user", false));
            ...
            if (cartModel == null) {
                ...
                regularAuditService.audit(new RegularAuditModel("Request to add product to cart",  email, "Failed to fetch the cart", false));
            ...
            if (productModel == null) {
                ...
                regularAuditService.audit(new RegularAuditModel("Request to add product to cart",  email, "Invalid product id given", false));
            ...
            if (productStock < cartStockRequired) {
                regularAuditService.audit(new RegularAuditModel("Request to add product to cart",  email, "Insufficent stock", false));
