...
public class SignupService {
    ...
    public ResponseEntity<?> saveUser(@Valid UserModel userModel) {
        ...
        regularAuditService.audit(
                new RegularAuditModel("Request for signup", userModel.getEmail(), "Product added to database", true));
        ...
        if (!emailExists || !userNameExists) {
            try {
                ...
                if (errors.size() > 0) {
                    regularAuditService.audit(new RegularAuditModel("Request for signup", userModel.getEmail(),
                            "Validation failed", false));
                    return new ResponseEntity<Object>(
                            new ApiResponse(false, "Validation of bean failed", FORBIDDEN.value(), FORBIDDEN, errors),
                            OK);
                }
            } catch (Exception e) {
                errors.add(e.getLocalizedMessage());
                regularAuditService.audit(
                        new RegularAuditModel("Request for signup", userModel.getEmail(), "Exception caused", false));
                return new ResponseEntity<Object>(
                        new ApiResponse(false, "There was an error", FORBIDDEN.value(), FORBIDDEN, errors), OK);
            }
        }
        try {
            if (emailExists && userNameExists) {
                ...
                regularAuditService.audit(new RegularAuditModel("Request for signup", userModel.getEmail(),
                        "User with email and username exists", false));
                return new ResponseEntity<Object>(
                        new ApiResponse(false, "A user with this email and username already exists",
                ...
                errors.add("Signup unsuccessful");
                regularAuditService.audit(new RegularAuditModel("Request for signup", userModel.getEmail(),
                        "User with email exists", false));
                return new ResponseEntity<Object>(new ApiResponse(false, "A user with this email already exists",
                        NOT_ACCEPTABLE.value(), NOT_ACCEPTABLE, errors), OK);
            } else if (userNameExists) {
                errors.add("User already exists");
                errors.add("Signup unsuccessful");
                regularAuditService.audit(new RegularAuditModel("Request for signup", userModel.getEmail(),
                        "User with username exists", false));
                return new ResponseEntity<Object>(new ApiResponse(false, "A user with this username already exists",
                        NOT_ACCEPTABLE.value(), NOT_ACCEPTABLE, errors), OK);
            } else {
                ...
                secret.replaceAll("-", "");
                UserModel temp = new UserModel(null, userModel.getEmail(), userModel.getPassword(),
                        userModel.getUsername(), userModel.getMobileNumber(), true, role, cart, null, userModel.isMfa(),
                        secret);
                ...
                regularAuditService.audit(new RegularAuditModel("Request for signup", userModel.getEmail(),
                        "Mail is sent after adding user", true));
                if (temp.isMfa()) {
                    HttpHeaders httpHeaders = new HttpHeaders();
                    httpHeaders.add("mfa", "true");
                    regularAuditService.audit(new RegularAuditModel("Request for signup", userModel.getEmail(),
                            "User opted for mfa / sending QR", true));
                    return ResponseEntity.ok().headers(httpHeaders)
                            .body(new ApiResponse(true,
                                    totpManager.getUriForImage(secret, temp.getEmail(), "EBook Store - Team 2"),
                                    OK.value(), OK, errors));
                }
            }
            ...
            if (e.getMessage().contains("constraint")) {
                ...
                errors.add("Signup unsuccessful");
                regularAuditService.audit(
                        new RegularAuditModel("Request for signup", userModel.getEmail(), "Constraint error", false));
                return new ResponseEntity<Object>(new ApiResponse(false, "There was a constraint error",
                        NOT_ACCEPTABLE.value(), NOT_ACCEPTABLE, errors), OK);
            }
        } catch (MessagingException e) {
            e.printStackTrace();
            regularAuditService.audit(new RegularAuditModel("Request for signup", userModel.getEmail(),
                    "Error while sending mail", false));
        ...
        regularAuditService.audit(
                new RegularAuditModel("Request for signup", userModel.getEmail(), "User creation successful", true));
        return new ResponseEntity<Object>(new ApiResponse(true, "User creation was successful", OK.value(), OK, errors),
                OK);
    }
